...
server.tool(
  ...
  {
    url: z.string().describe('The Linear upload/attachment URL to download'),
    savePath: z.string().optional().describe('Where to save the file (default: /workspace/group/downloads/<filename>)'),
  },
  async (args) => {
    try {
      ...
      if (!res.ok) {
        return {
          content: [{ type: 'text' as const, text: `Download failed: ${res.status} ${res.statusText}` }],
          isError: true,
        };
      }
      ...
      const defaultName = path.basename(urlPath) || 'attachment';
      const savePath = args.savePath || path.join('/workspace/group/downloads', defaultName);
      ...
      return {
        content: [{ type: 'text' as const, text: `Downloaded to ${savePath} (${buffer.length} bytes). Use the Read tool to view it.` }],
      };
    } catch (err) {
      return {
        content: [{ type: 'text' as const, text: `Error downloading: ${err instanceof Error ? err.message : String(err)}` }],
        isError: true,
      };
    }
  },
